...
for cmd in python3 sha256sum msgfmt msgcmp gettext cmp; do
...
for path in "$METADATA" "$EXTENSION_JS" "$PREFS_JS" "$PO" "$POT" "$TARGET_MO"; do
    [[ -f "$path" ]] || {
...
if ! cmp -s "$tmpdir/advanced-media-controller.mo" "$TARGET_MO"; then
    echo "FAIL: installed Advanced Media Controller Polish catalog differs from repository" >&2
    exit 1
fi

translated="$(TEXTDOMAINDIR="$EXT_DIR/locale" TEXTDOMAIN="$EXPECTED_DOMAIN" LANGUAGE="pl_PL:pl" gettext "General")"
[[ "$translated" == "Ogólne" ]] || {
    echo "FAIL: Advanced Media Controller gettext runtime test failed: expected 'Ogólne', found '$translated'" >&2
    exit 1
}

printf 'Translated entries: %d\n' "$entries"
echo "PASS: Advanced Media Controller v31 / 6.5 Polish localization matches repository"

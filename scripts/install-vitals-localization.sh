...
for path in "$METADATA" "$SENSORS" "$PREFS_JS" "$PREFS_UI" "$OVERLAY" "$TARGET_MO" "$VERIFIER"; do
    if [[ ! -f "$path" ]]; then
...
if [[ ! -f "$BACKUP_MO" ]]; then
    ...
    if [[ "$current_sha" != "$EXPECTED_UPSTREAM_MO_SHA" ]]; then
        echo "FAIL: Vitals v85 Polish catalog is not the audited upstream artifact; refusing to overwrite" >&2
...
install -m 0644 "$tmpdir/vitals.mo" "$TARGET_MO"

bash "$VERIFIER"
echo "PASS: Vitals v85 Polish completion installed"
echo "Sign out and back in to reload Vitals runtime translations."

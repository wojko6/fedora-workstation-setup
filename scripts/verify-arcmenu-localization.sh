...
for path in "$EXT_DIR/metadata.json" "$CONSTANTS" "$MENU_WIDGETS" "$POLISH_MO" "$BACKUP" "$PATCH" "$SOURCE"; do
    [[ -f "$path" ]] || {
        echo "FAIL: required ArcMenu v74 localization verification file missing: $path" >&2
        exit 1
    }
done
...
cmp -s "$tmp/constants.js" "$CONSTANTS" || {
    echo "FAIL: ArcMenu v74 Polish gettext binding fix differs from repository" >&2
    exit 1
}
...
for path in "$CONSTANTS" "$PATCH"; do
    if grep -Fq '/home/wojciech' "$path"; then
        echo "FAIL: ArcMenu v74 localization contains a private hardcoded path: $path" >&2
        exit 1
    fi
done

echo "PASS: ArcMenu v74 / 70.0 Polish gettext binding fix matches repository"

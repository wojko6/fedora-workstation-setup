...
for cmd in rpm python3 msgfmt msgunfmt msgcat cmp sudo; do
...
[[ -d "$EXT_DIR" ]] || {
    echo "FAIL: AppIndicator extension directory missing: $EXT_DIR" >&2
    exit 1
}
...
[[ -f "$OVERLAY_PO" ]] || {
    echo "FAIL: AppIndicator Polish completion overlay missing: $OVERLAY_PO" >&2
    exit 1
}
[[ -f "$TARGET_MO" ]] || {
    echo "FAIL: upstream AppIndicator Polish catalog missing: $TARGET_MO" >&2
    exit 1
}
...
if [[ ! -f "$BACKUP_MO" ]]; then
    echo "Creating version-specific upstream catalog backup..."
...
sudo install -m 0644 "$expected_mo" "$TARGET_MO"

if ! cmp -s "$expected_mo" "$TARGET_MO"; then
    echo "FAIL: installed AppIndicator Polish catalog differs from expected merge" >&2
    exit 1
fi

echo "Completion entries: $overlay_entries"
echo "PASS: AppIndicator v64 Polish completion installed"
echo "Close and reopen AppIndicator preferences before visual validation."

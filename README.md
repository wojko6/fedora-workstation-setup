...
The workstation now also defines a dedicated `workstation-tailscale` firewalld zone for `tailscale0`. Its exact policy is `target=DROP` with no allowed services, ports, protocols, sources, forwarding, masquerade, ICMP inversion, or rich rules. This prevents Tailscale traffic from falling through to Fedora's broader `FedoraWorkstation` default-zone policy while preserving outbound/established Tailscale connectivity. A live 2026-09-24 A/B test changed remote TCP/1716 and TCP/27036 from `open` to `filtered`, and the dedicated-zone assignment persisted across reboot. The previously accepted `PASS=256` aggregate predates the repository-managed version of this control; a fresh full verifier run is required before promoting a new aggregate.

...
- **ArcMenu v74 / 70.0**: the exact EGO v74 runtime was re-audited after the extension updated from v73. The upstream Polish catalog remained usable, but the runtime needed explicit `bindtextdomain()` binding to the extension-local locale directory. The production tree was visually confirmed with Polish power-button tooltips and pinned at `d89199fe6cd1da1bdc5d834532ab12aee5f7c64725ad121e038b862569ab3148`.
- **GSConnect v73**: the 20-entry managed catalog, gettext-domain repairs, and five factory RunCommand display-name translations were restored and verified. The producer's four-form plural layout is retained intentionally for compatibility; it is not normalized to the usual three-form Polish header. The accepted extension tree is `1aa60d482b729a83d46f08dc4796bab3d3f9a66fe100e07fb3cdb95d21505f10`.
- **DING v97**: visual inspection exposed remaining English labels in the desktop background menu and its Arrange By submenu even though the earlier System Monitor integration already passed. The managed catalog now covers the full observed menu, including `Nowy folder`, paste/select/arrange actions, Files/background/settings actions, and all sorting choices. The final visually accepted DING tree is `f1ec256c26a42371ab80cd331a82efafe42766c1d5608a953fad14fa8512ca73`.
- **Helium 0.18.1.1**: the version change from 0.17.2.1 triggered a fresh fail-closed audit rather than a version-only pin update. The same 36 Linux-relevant untranslated names remain valid, their Chromium 154 resource IDs were re-audited, and strict reconstruction matches the installed patched DataPack byte-for-byte.
- One unrelated Space Bar `application-styles` desired-state drift was restored to the already accepted repository CSS rather than promoted.

The final physical `scripts/verify.sh` summary after these changes is `PASS=256 WARN=0 FAIL=0 SKIP=0`. PR #34 was squash-merged to `main` as `814907f5932c321ecf52f72b1708ff787d18d081`. Issue #14 remains open as the recurring package/extension/localization maintenance tracker.
...
- **Helium 0.18.1.1 / Chromium 154.0.8037.57**: the physical Fedora COPR RPM ships a Polish Chromium DataPack v5 at `/opt/helium/locales/pl.pak`. Re-audit of the exact upstream 0.18.1 source finds the same 38 Polish source gaps as before, of which two are macOS-only and the same 36 names are Linux-relevant. The v0.18.1.1 producer backup SHA-256 is `211933686949bd514ec57a450bd7f4b39631a474c891b70e1214202de9f34c02`; applying the 36 reviewed translations to that pristine DataPack reports `patched=36 already=0 drift=0` and reconstructs the live patched SHA-256 `2cb7a4199af218f9b0e0976b33e5f7653510fb6b7c5b7796606343cdb85ce579` byte-for-byte. The exact Chromium 154 resource IDs were re-pinned from the v0.18.1.1 producer `.pak.info` rather than weakening strict verification. The repository pins Helium source commit `b38c4bdd2ecbe5c680dc3c5d464a2edc84d49d4c` and Linux packaging commit `69c98cfe6cad9e19143f531211a5a427c8561625`. Static validation, strict installer reconstruction, the dedicated verifier, DNF5 persistence verification, and the final full physical verifier all pass.

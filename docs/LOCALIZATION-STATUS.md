...
- Ptyxis 50.1 Polish main-window/menu, terminal-context, search, inspector, title-dialog, search-options, libadwaita About-dialog, and GNOME Shell desktop-action localization
...
The audit also exposed a verifier-design issue in the Ptyxis search-options completion: the actual embedded resource uses mnemonic-bearing msgids `Match _Case`, `Whole _Words`, and `Use _Regular Expressions`. The repository catalog and verifier have been corrected to those exact resource strings, and the verifier now checks the installed Ptyxis resource before accepting the translations. Physical inspection on 2026-09-24 confirmed those in-application search options and the remaining audited in-application Ptyxis UI areas in Polish. A later GNOME application-grid check exposed three English desktop actions: `New Window`, `New Tab`, and `Preferences`. The exact package build `ptyxis-50.1-2.fc44` owns `/usr/share/applications/org.gnome.Ptyxis.desktop`; the pristine file was verified unmodified with SHA-256 `8c596c2aff40ac062f61e6c541f0bccfa62091ce8503d63e2306d2e0a97f2b88`. The repository now has a deterministic fail-closed patcher and verifier that add only `Name[pl]=Nowe okno`, `Name[pl]=Nowa karta`, and `Name[pl]=Preferencje` while preserving the audited action commands. Physical installation and final visual acceptance of these three launcher labels are still required.

...
- Helium 0.18.1.1 / Chromium 154.0.8037.57 uses the re-audited 36-entry Linux-relevant JSON completion applied to Chromium DataPack v5 `pl.pak`; all 36 prior runtime source strings still match, the exact missing-name set remains unchanged, and the patcher preserves unmanaged resources while enforcing the audited producer fingerprint in strict mode.

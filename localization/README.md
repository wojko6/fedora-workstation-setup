...
All localization installation is centralized through `scripts/install-localizations.sh`: 15 version-specific GNOME-extension installers plus Papers, Plymouth, Ptyxis, Extension Manager, Helium, and GNOME Tweaks application/system stages (21 scripted localization stages total). Verification is integrated into `scripts/verify.sh`. VSCodium is intentionally deferred and is not part of the current reproducible localization set.
